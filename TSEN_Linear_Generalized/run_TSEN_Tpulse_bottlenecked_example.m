...
% (with a single bottleneck intermediate reaction) undergoing a temperature
% pulse
...
saving = false;
saveprefix = 'bottlenecked_TSEN_Tpulse';
...
T1 = 37; % Temperature BEFORE pulse
T2 = 27; % Temperature DURING pulse
T3 = 37; % Temperature AFTER pulse
...
dt = 0.2;
Nsteps = .5*1.0E4;
Npulse = round(Nsteps*.2); % Set Npulse as fraction of Nsteps

%% Run simulation using Tpulse simulator 
[V,C,time,growthrate] = TSEN_Generalized_Tpulse_simulator(T1,T2,T3,c0,I,P,G,gamma0,dt,Nsteps,Npulse);
